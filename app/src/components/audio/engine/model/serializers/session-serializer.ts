...
export function serialize(session: Session): SerializedSession {
  return {
    ...
    channels: session.channels.map((c) => ({
      ...
      enabled:         c.enabled,
      hostInstrument:  c.hostInstrument,      // null when WebAudio-driven
      fxChain:         c.fxChain,             // [] when no host FX
...
export function fromEffluxSong(song: EffluxSong): Session {
  ...
  for (let c = 0; c < channelCount; c++) {
    ...
    channels[c] = {
      ...
      playingCursor:   0,
      enabled:         true,
      hostInstrument:  null,                  // XTK has no host concept
      fxChain:         [],
    };
  }
...
export function createBlankSession(
  ...
  for (let c = 0; c < instruments.length; c++) {
    ...
    channels[c] = {
      ...
      playingCursor:   0,
      enabled:         true,
      hostInstrument:  null,                  // blank = WebAudio-driven
      fxChain:         [],
    };
  }
...
function parseChannelState(
  ...
  const hostInstrument = parsePluginRefOrNull(raw.hostInstrument, channelIdx);
  const fxChain        = parseFxChain(raw.fxChain, channelIdx);
...
/**
 * Parse a host plugin reference. Append-only at v1: absent / null reads as
 * null (channel is WebAudio-driven). Present-but-malformed throws — silently
 * dropping a partially-written hostInstrument would resurrect the WebAudio
 * synth on a session the user authored against the host, which is worse than
 * a clear load error.
 */
function parsePluginRefOrNull(raw: unknown, channelIdx: number): PluginRef | null {
  if (raw === undefined || raw === null) return null;
  if (!isObject(raw)) {
    throw new SessionFormatError(`channels[${channelIdx}].hostInstrument must be an object or null`);
  }
  return {
    uid:      strField(raw, 'uid'),
    path:     strField(raw, 'path'),
    name:     strField(raw, 'name'),
    bypassed: boolField(raw, 'bypassed', false),    // append-only at v1
  };
}

/**
 * Parse an fxChain array. Append-only at v1: absent reads as []. Each element
 * is a strict PluginRef — an empty/null entry is a malformed file, not an
 * empty slot (slots are positional in the array, not nullable).
 */
function parseFxChain(raw: unknown, channelIdx: number): PluginRef[] {
  if (raw === undefined) return [];
  if (!Array.isArray(raw)) {
    throw new SessionFormatError(`channels[${channelIdx}].fxChain must be an array`);
  }
  const out: PluginRef[] = new Array(raw.length);
  for (let i = 0; i < raw.length; i++) {
    const entry = raw[i];
    if (!isObject(entry)) {
      throw new SessionFormatError(`channels[${channelIdx}].fxChain[${i}] must be an object`);
    }
    out[i] = {
      uid:      strField(entry, 'uid'),
      path:     strField(entry, 'path'),
      name:     strField(entry, 'name'),
      bypassed: boolField(entry, 'bypassed', false),  // append-only at v1
    };
  }
  return out;
}
