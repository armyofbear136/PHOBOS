/**
 * SculptGLPanel.tsx — Fullscreen SculptGL sculpting editor panel.
 *
...
const SCULPTGL_URL = 'http://localhost:16348';
const POLL_MS      = 3_000;

type SculptGLState = 'stopped' | 'starting' | 'running' | 'error';

interface SculptGLStatus {
  state:        SculptGLState;
  port:         number;
  error:        string | null;
...
function useSculptGLStatus() {
  const [status,  setStatus]  = useState<SculptGLStatus | null>(null);
  const [loading, setLoading] = useState(false);
  const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const fetchStatus = useCallback(async () => {
    try {
      const res = await fetch(`${ENGINE_URL}/api/tools/sculptgl/status`);
  ...
  const start = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${ENGINE_URL}/api/tools/sculptgl/start`, { method: 'POST' });
      if (res.ok) setStatus(await res.json());
...
export function SculptGLPanel() {
  ...
  return (
    <div className={`fixed inset-x-0 bottom-0 top-10 z-40 flex flex-col bg-background ${
      sculptglPanelOpen ? '' : 'invisible pointer-events-none'
    }`}>
      ...
      <div className="flex-1 min-h-0 relative">
        {starting && !running && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-background">
            ...
            <span className="text-xs font-mono text-muted-foreground/40">Starting SculptGL…</span>
          </div>
        )}
        {notInstalled && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-background">
            <Boxes className="w-8 h-8 text-phobos-green/20" />
            <span className="text-sm font-mono text-muted-foreground/50">SculptGL not installed</span>
            <span className="text-[10px] font-mono text-muted-foreground/30">
              Run <code className="text-phobos-green/40">node scripts/fetch-sculptgl.js</code>
            </span>
          </div>
        )}
