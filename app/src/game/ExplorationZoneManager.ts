...
export interface RoomInstance {
  ...
  /**
   * Synthetic suppression strips added by generateZoneGraph at inter-region seams.
   * Each entry is a world-tile centre point + edge direction — treated identically
   * to a connection slot for the purpose of skipping structure tile rendering.
   * Ensures the exit face of terminal rooms has no visual wall tiles.
   */
  seamStrips?: Array<{ worldTx: number; worldTy: number; edge: EdgeDir }>;
}
...
// ── Zone catalogue ────────────────────────────────────────────────────────
//
// All zone data (rooms, regions, zone defs) lives in app/src/game/map/.
// One file per zone, one folder per act. map/index.ts aggregates everything
// into the three flat arrays re-exported below.
//
// To add a zone: create map/actN/ANZxx_Name.ts, add three spread lines to
// map/index.ts. Nothing else changes here.

import { ROOM_CATALOGUE, REGION_REGISTRY, ZONE_LIBRARY } from './map/index';
export { ROOM_CATALOGUE, REGION_REGISTRY, ZONE_LIBRARY };
...
function assembleSpineRegion(
  ...
  // middlePool: through-rooms only — dead-ends must never appear in the middle of a spine.
  // finalPool: prefer terminals (dead-ends) so the last room feels like a destination;
  //            fall back to through-rooms if no terminals exist for this region.
  const middlePool  = throughRooms.length > 0 ? throughRooms : [];
  const finalPool   = terminalRooms.length > 0 ? terminalRooms : throughRooms;
  ...
  for (let i = 0; i < Math.max(0, middleCount - 1); i++) {
    if (middlePool.length === 0) break;
    selectedRoomDefs.push(middlePool[Math.floor(rng() * middlePool.length)]);
  }
  // Last non-boss room: prefer terminal (dead-end), fall back to through-room.
  if (middleCount >= 1 && finalPool.length > 0) {
...
// ── generateZoneGraph ────────────────────────────────────────────────────
//
// Assembles a full ZoneGraph from ZONE_LIBRARY. Zone selection is act-based:
// the daily seed determines the act (1–5), and within that act the zone is
// picked by seed % act_zone_count. Falls back to any available zone if the
// selected act has no entries yet (expected during incremental authoring).

export function generateZoneGraph(seed: number): ZoneGraph {
  const rng = makeLcg(seed);

  // Pick zone def — select by act derived from seed, then by position within that act.
  // act = Math.floor(archetypeId / 20), so act 1 = ids 0–19, act 2 = ids 20–39, etc.
  // Daily seed determines which act's zones are active; within the act, seed picks the zone.
  // Falls back to the first available zone if the act has no entries yet.
  const actId    = Math.floor((seed % 100) / 20) + 1;  // 1–5
  const actZones = ZONE_LIBRARY.filter(z => z.zone_act === actId);
  const pool     = actZones.length > 0 ? actZones : ZONE_LIBRARY;
  const zoneDef  = pool[seed % pool.length];
  ...
  // Tag entry rooms of each region (except the first) with a seam strip at their
  // south face. When two regions connect directly (no corridor between them), the
  // south wall of the entry room sits exactly at the seam tile and must be
  // visually suppressed so the passage is open.
  for (let ri = 1; ri < regions.length; ri++) {
    const region = regions[ri];
    if (region.rooms.length === 0) continue;
    const entryRoom = region.rooms[0];
    const seamTx = region.entryTx;
    const seamTy = region.entryTy;
    if (!entryRoom.seamStrips) entryRoom.seamStrips = [];
    entryRoom.seamStrips.push({
      worldTx: seamTx,
      worldTy: seamTy,
      edge:    'S',
    });

    // Also tag the exit room of the previous region at its north face.
    const prevRegion = regions[ri - 1];
    if (prevRegion.rooms.length > 0) {
      const exitRoom = prevRegion.rooms.find(r =>
        prevRegion.exitTx >= r.worldOffsetTx &&
        prevRegion.exitTx <  r.worldOffsetTx + r.def.size.w &&
        prevRegion.exitTy >= r.worldOffsetTy &&
        prevRegion.exitTy <= r.worldOffsetTy + r.def.size.h,
      ) ?? prevRegion.rooms[prevRegion.rooms.length - 1];
      if (!exitRoom.seamStrips) exitRoom.seamStrips = [];
      exitRoom.seamStrips.push({
        worldTx: prevRegion.exitTx,
        worldTy: prevRegion.exitTy,
        edge:    'N',
      });
    }
  }
