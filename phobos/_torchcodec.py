...
def load_with_torchcodec(
    ...
    Note:
        ...
        - Not all audio formats supported by torchaudio backends may be supported
          by TorchCodec.
    """
    # Import torchcodec — fall back to soundfile if FFmpeg DLLs are unavailable
    try:
        from torchcodec.decoders import AudioDecoder
    except Exception:
        # torchcodec unavailable (e.g. missing FFmpeg DLLs on Windows portable installs)
        # Fall back to soundfile which needs no FFmpeg.
        import soundfile as sf
        import numpy as np
        data, sample_rate = sf.read(uri, dtype="float32", always_2d=True)
        waveform = torch.from_numpy(data.T)  # (frames, ch) -> (ch, frames)
        if frame_offset > 0:
...
def save_with_torchcodec(
    ...
    try:
        ...
        import soundfile as sf
        import numpy as np
        data = src if src.ndim == 2 else src.unsqueeze(0)
        if not channels_first:
            data = data.T
        else:
            data = data.T  # (ch, frames) -> (frames, ch) for soundfile
        sf.write(str(uri), data.cpu().numpy(), sample_rate)
        return
