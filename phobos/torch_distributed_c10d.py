...
try:
    ...
    # Provide no-op stubs so this module loads; distributed ops are
    # never used in single-GPU training on the 890M.
    ...
    _DistributedBackendOptions = _NoOp
    _register_process_group = _NoOp()
    _resolve_process_group = _NoOp()
    _unregister_all_process_groups = _NoOp()
    _unregister_process_group = _NoOp()
    AllgatherOptions = _NoOp
    AllreduceCoalescedOptions = _NoOp
    AllreduceOptions = _NoOp
    AllToAllOptions = _NoOp
    BarrierOptions = _NoOp
    BroadcastOptions = _NoOp
    DebugLevel = _NoOp
    GatherOptions = _NoOp
    get_debug_level = _NoOp()
    PrefixStore = _NoOp
    ProcessGroup = _NoOp
    ReduceOp = _ReduceOp
    ReduceOptions = _NoOp
    ReduceScatterOptions = _NoOp
    ScatterOptions = _NoOp
    Store = _NoOp
    Work = _NoOp
