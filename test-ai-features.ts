...
// ── HTTP helpers ───────────────────────────────────────────────────────────────
function apiPost(path: string, body: unknown, timeoutMs = 10_000): Promise<{ status: number; json: unknown }> {
...
async function testS01_intentRoutingDirect(): Promise<void> {
  ...
  const s   = await sendMessageAndCapture(tid, 'What is a closure in JavaScript?', {}, 30_000);
...
async function testS02_intentRoutingNeeds(): Promise<void> {
  ...
  const tid = randomUUID();
  const s   = await sendMessageAndCapture(tid, 'Write a TypeScript function that reverses a string', {}, 60_000);
...
async function testS03_ctxComputedEvent(): Promise<void> {
  ...
  const s1 = await sendMessageAndCapture(tid, 'Hello, what can you do?', {}, 30_000);
  ...
  assert('first message count is 0',     s1.ctxCount === 0,        'no prior history yet');
  assert('complete received',            s1.complete,              'stream done');

  // Second message — should now have 1 prior pair
  const s2 = await sendMessageAndCapture(tid, 'What is two plus two?', {}, 30_000);
...
async function testS04_distilledContentUsedForContext(): Promise<void> {
  ...
  const s1 = await sendMessageAndCapture(tid, 'Write a TypeScript function called add that takes two numbers and returns their sum', {}, 60_000);
...
async function testS05_conversationTurnIndexed(): Promise<void> {
  ...
  const tid = randomUUID();
  const s   = await sendMessageAndCapture(tid, 'We should always use const instead of let where possible in TypeScript', {}, 30_000);
...
async function testS07_distillationStrip(): Promise<void> {
  ...
  const tid = randomUUID();
  const s   = await sendMessageAndCapture(tid, 'Write a function called multiply in TypeScript. Return the code in a fenced block.', {}, 60_000);
  assert('complete', s.complete, `elapsed ${s.durationMs}ms`);
...
async function testS08_clarificationLoopSeren(): Promise<void> {
  ...
  const s1  = await sendMessageAndCapture(tid, 'fix the bug', {}, 60_000);
  ...
  if (s1.clarification) {
    ...
    const s2 = await sendMessageAndCapture(tid, 'I meant fix the null pointer error in the login handler', {}, 60_000);
...
async function testS09_ctxOverrideManual(): Promise<void> {
  ...
  for (let i = 0; i < 4; i++) {
    await sendMessageAndCapture(tid, `Message ${i + 1}: tell me something interesting about prime numbers`, {}, 30_000);
  }

  // Now send with explicit depth of 2
  const s = await sendMessageAndCapture(tid, 'Summarise what we discussed', { contextHistoryDepth: 2 }, 30_000);
...
async function testS10_archiveDoesNotPolluteCovnersations(): Promise<void> {
  ...
  const s = await sendMessageAndCapture(tid, 'What does the documentation say about API rate limits?', {}, 30_000);
...
async function testM01_directAnswerPersistence(): Promise<void> {
  ...
  const tid = randomUUID();
  const s   = await sendMessageAndCapture(tid, 'Explain the difference between a mutex and a semaphore in two sentences.', {}, 45_000);
...
async function testM06_copilotSayonMemory(): Promise<void> {
  ...
  const r1 = await apiPost('/api/copilot/sayon', {
    content: 'My preferred coding language is TypeScript and I always use strict mode.',
  });
  // Copilot returns SSE inline — the POST response is the stream
  // For simplicity, just check the HTTP response came back
  assert('copilot POST responds', (r1.status === 200 || r1.status === 0), `status: ${r1.status}`);
...
async function main(): Promise<void> {
  ...
  if (runShort) {
    console.log('\n── Short Tests (S01–S10) ─────────────────────────────────────');
    if (!ONLY_ID || ONLY_ID === 'S01') await testS01_intentRoutingDirect();
    if (!ONLY_ID || ONLY_ID === 'S02') await testS02_intentRoutingNeeds();
    if (!ONLY_ID || ONLY_ID === 'S03') await testS03_ctxComputedEvent();
    if (!ONLY_ID || ONLY_ID === 'S04') await testS04_distilledContentUsedForContext();
    if (!ONLY_ID || ONLY_ID === 'S05') await testS05_conversationTurnIndexed();
    if (!ONLY_ID || ONLY_ID === 'S06') await testS06_memoryIntentPatterns();
    if (!ONLY_ID || ONLY_ID === 'S07') await testS07_distillationStrip();
    if (!ONLY_ID || ONLY_ID === 'S08') await testS08_clarificationLoopSeren();
    if (!ONLY_ID || ONLY_ID === 'S09') await testS09_ctxOverrideManual();
    if (!ONLY_ID || ONLY_ID === 'S10') await testS10_archiveDoesNotPolluteCovnersations();
  }
  ...
  if (runMedium && !SHORT_ONLY) {
    ...
    if (!ONLY_ID || ONLY_ID === 'M01') await testM01_directAnswerPersistence();
    if (!ONLY_ID || ONLY_ID === 'M02') await testM02_singleFileCreation();
    if (!ONLY_ID || ONLY_ID === 'M03') await testM03_conversationRAGRetrieval();
    if (!ONLY_ID || ONLY_ID === 'M04') await testM04_rollingChatSummary();
    if (!ONLY_ID || ONLY_ID === 'M05') await testM05_multiTaskDependencies();
    if (!ONLY_ID || ONLY_ID === 'M06') await testM06_copilotSayonMemory();
    if (!ONLY_ID || ONLY_ID === 'M07') await testM07_inlineContentExtraction();
    if (!ONLY_ID || ONLY_ID === 'M08') await testM08_fileReadCycle();
  }
  ...
  if (runLong && !SHORT_ONLY) {
    console.log('\n── Long Tests (L01–L05) ──────────────────────────────────────');
    if (!ONLY_ID || ONLY_ID === 'L01') await testL01_paginatedWriting();
    if (!ONLY_ID || ONLY_ID === 'L02') await testL02_agentStateSequence();
    if (!ONLY_ID || ONLY_ID === 'L03') await testL03_contextOverflowHandling();
    if (!ONLY_ID || ONLY_ID === 'L04') await testL04_fullConversationRAGRoundTrip();
    if (!ONLY_ID || ONLY_ID === 'L05') await testL05_exhaustiveWebsite();
  }
  ...
  if (runEdge && !SHORT_ONLY) {
    ...
    if (!ONLY_ID || ONLY_ID === 'E01') await testE01_sybilUnavailableDegrades();
    if (!ONLY_ID || ONLY_ID === 'E02') await testE02_emptyContentRejected();
    if (!ONLY_ID || ONLY_ID === 'E03') await testE03_fileOverwriteSafe();
